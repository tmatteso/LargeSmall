...
def is_ref_allele(allele, ref_dict_row):
    if allele == ref_dict_row[3]:
        return True
    return False
...
def allele_in_large(allele, alleles, ref_dict_row):
    if allele in translate_extra_symbols(alleles[4][1]):
        return True
    elif is_ref_allele(allele, ref_dict_row):
        return True
    return False


# simply checks if this particular allele exists in the sm founder
def allele_in_small(allele, alleles, ref_dict_row):
...
def generate_new_allele_map(alleles, allele_list, ref_dict_row):
    new_chrom_list = []
    for allele in allele_list:
        if allele_in_large(allele, alleles, ref_dict_row) and allele_in_small(allele, alleles, ref_dict_row):
            new_chrom_list.append("B")
        elif allele_in_large(allele, alleles, ref_dict_row):
            new_chrom_list.append("L")
        elif allele_in_small(allele, alleles, ref_dict_row):
...
def declare_allele_class(ref_dict_row, sample_dict_row):
    ...
    for sample in range(len(sample_dict_row)):
        ...
        new_chrom_map = generate_new_allele_map(ref_dict_row, allele_list, ref_dict_row)
